from .exceptions import MissingJwksClaim
from .exceptions import MissingTrustMark
from .exceptions import TrustAnchorNeeded
...
def get_federation_jwks(jwt_payload: dict, httpc_params: dict = {}):
    return jwt_payload.get("jwks", {}).get("keys", [])
...
class TrustMark:
    ...
    def validate_by_its_issuer(self) -> bool:
        ...
        payload = verify_jws(
            self.jwt,
            ec.jwks[ec.kids.index(self.header["kid"])],
        )
        self.is_valid = True
        return payload
